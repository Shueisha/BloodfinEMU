...
void ChatManagerImplementation::initiateRooms() {
	...
	galaxyRoom = createRoom("Galaxy", core3Room);
	core3Room->addSubRoom(galaxyRoom);
...
void ChatManagerImplementation::handleSocialInternalMessage(CreatureObject* sender, const UnicodeString& arguments) {
	if (sender->isPlayerCreature()) {
		...
		if (senderGhost->isMuted()) {
			...
			if (reason != "")
				sender->sendSystemMessage("Enjoy the music while you wait - chat abilities are currently disabled by Bloodfin Customer Support for '" + reason + "'.");
			else
				sender->sendSystemMessage("Enjoy the music while you wait - chat abilities are currently disabled by Bloodfin Customer Support.");
...
void ChatManagerImplementation::handleSpatialChatInternalMessage(CreatureObject* player, const UnicodeString& args) {
	if (player->isPlayerCreature()) {
		...
		if (ghost->isMuted()) {
			...
			if (reason != "")
				player->sendSystemMessage("Enjoy the music while you wait - chat abilities are currently disabled by Bloodfin Customer Support for '" + reason + "'.");
			else
				player->sendSystemMessage("Enjoy the music while you wait - chat abilities are currently disabled by Bloodfin Customer Support.");
...
void ChatManagerImplementation::handleGalaxyChat(CreatureObject* sender, const UnicodeString& message) {
	if (sender->isPlayerCreature()) {
		ManagedReference<PlayerObject*> senderGhost = sender->getPlayerObject();
	...
	if (galaxyRoom != NULL) {
		BaseMessage* msg = new ChatRoomMessage(name, message, galaxyRoom->getRoomID());
		galaxyRoom->broadcastMessage(msg);
	}
