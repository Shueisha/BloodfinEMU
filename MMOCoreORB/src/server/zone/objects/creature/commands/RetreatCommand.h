...
//		shoutCommand(player, group);
		...
		for (int i = 1; i < group->getGroupSize(); ++i) {
			...
			// Member is invalid target
			if (member == NULL || !member->isPlayerCreature() || member->getZone() != creature->getZone())
				continue;

			// Member is further than 64 meters from Scout Leader
			if(member->getDistanceTo(player) > 40)
				continue;
	...
	void doRetreat(CreatureObject* player) {
		...
		// Player already has the buff or another speed increasing buff
		if (player->hasBuff(actionCRC) || player->hasBuff(BuffCRC::JEDI_FORCE_RUN_1) || player->hasBuff(BuffCRC::JEDI_FORCE_RUN_2) || player->hasBuff(BuffCRC::JEDI_FORCE_RUN_3))
			return;
