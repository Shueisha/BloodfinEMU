...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		//Rank Bonus
		int rankMod = 1;
		int rankModD = (creature->getSkillMod("force_control_dark") * 10);
		int rankModL = (creature->getSkillMod("force_control_light") * 10);
		...
		uint32 healthHealed = creature->healDamage(creature, CreatureAttribute::HEALTH, 1500 + rankMod);
		uint32 actionHealed = creature->healDamage(creature, CreatureAttribute::ACTION, 1500 + rankMod);
		uint32 mindHealed = creature->healDamage(creature, CreatureAttribute::MIND, 1500 + rankMod);

			
		forceCost = MIN((((rankMod * 3) - healthHealed + actionHealed + mindHealed) / 9.5), 470);
