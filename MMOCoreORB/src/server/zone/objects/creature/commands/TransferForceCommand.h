...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		int forcePower = targetGhost->getForcePower();
		int forcePowerMax = targetGhost->getForcePowerMax();

		if(forcePower >= forcePowerMax){
			creature->sendSystemMessage("Your target is at maximum Force power!");
			targetGhost->setForcePower(forcePowerMax);
			return INVALIDTARGET;}

		if (playerObject->getForcePower() <= 200){
			creature->sendSystemMessage("You don't have enough Force power to transfer!");
			return INVALIDTARGET;
		}
