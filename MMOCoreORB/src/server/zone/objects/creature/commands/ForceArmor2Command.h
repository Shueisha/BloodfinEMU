...
public:
	...
	void handleBuff(SceneObject* creature, ManagedObject* object, int64 param) {
		...
		//int forceCost = param * 0.2;
		int rankModL = creo->getSkillMod("force_manipulation_light");
		int rankModD = creo->getSkillMod("force_manipulation_dark");
		int rankMod = 1;

		if(rankModL > rankModD)
			rankMod = (creo->getSkillMod("force_manipulation_light") / 2.f);

		if(rankModD > rankModL)
			rankMod = (creo->getSkillMod("force_manipulation_dark") / 2.f);

		float forceCost = (param / rankMod) * 0.15;

		if (playerObject->getForcePower() <= forceCost)
		{ // Remove buff if not enough force.
