...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
        ...
        if (creature->getDistanceTo(object) > 32.f){
            ...
            return GENERALERROR;}
		int rankMod = 1;
		int rankModD = (creature->getSkillMod("force_power_dark") * 25);
		int rankModL = (creature->getSkillMod("force_control_light") * 25);
		...
		creatureTarget->healWound(creature, CreatureAttribute::ACTION, 350 + rankMod);

		creatureTarget->healDot(CreatureState::DISEASED, 1300);

		creatureTarget->healDot(CreatureState::POISONED, 1300);

		creatureTarget->healDot(CreatureState::BLEEDING, 1300);

		creatureTarget->healWound(creature, CreatureAttribute::QUICKNESS, 350 + rankMod);

		creatureTarget->healWound(creature, CreatureAttribute::STAMINA, 350 + rankMod);

		creatureTarget->healWound(creature, CreatureAttribute::HEALTH, 350 + rankMod);

		creatureTarget->healWound(creature, CreatureAttribute::STRENGTH, 350 + rankMod);

		creatureTarget->healWound(creature, CreatureAttribute::CONSTITUTION, 350 + rankMod);

		creatureTarget->healWound(creature, CreatureAttribute::MIND, 350 + rankMod);

		creatureTarget->healWound(creature, CreatureAttribute::FOCUS, 350 + rankMod);

		creatureTarget->healWound(creature, CreatureAttribute::WILLPOWER, 350 + rankMod);
		...
		creatureTarget->healDot(CreatureState::ONFIRE,1300);
		...
		forceCost = MIN(((healedHealth + healedAction + healedMind - (rankMod * 2)) / 5), 470);
