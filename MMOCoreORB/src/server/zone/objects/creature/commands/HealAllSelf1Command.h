...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		if (isWearingArmor(creature) && !creature->getSkillMod("jediarmor") > 0) {
			return NOJEDIARMOR;
		}
		...
		//Rank Bonus
		int rankMod = 1;
		int rankModD = (creature->getSkillMod("force_control_dark") * 5);
		int rankModL = (creature->getSkillMod("force_control_light") * 5);
		...
		uint32 healthHealed = creature->healDamage(creature, CreatureAttribute::HEALTH, 500 + rankMod);
		uint32 actionHealed = creature->healDamage(creature, CreatureAttribute::ACTION, 500 + rankMod);
		uint32 mindHealed = creature->healDamage(creature, CreatureAttribute::MIND, 500 + rankMod);

			
		forceCost = MIN((((rankMod * 3) - healthHealed + actionHealed + mindHealed) / 4), 340);
