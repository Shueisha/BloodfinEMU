...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		try {
			...
			if (dir != "roll" && dir != "yaw" && dir != "pitch")
				throw Exception();

		} catch (Exception& e) {
			creature->sendSystemMessage("Format: /rotateFurniture <ROLL/YAW/PITCH> <degrees>"); //Format: /rotateFurniture <LEFT/RIGHT> <degrees>
			return INVALIDPARAMETERS;
		}

		if (degrees < -360 || degrees > 360) {
			creature->sendSystemMessage("Value must be between -360 and 360."); //The amount to rotate must be between 1 and 180.
			return INVALIDPARAMETERS;
		}
