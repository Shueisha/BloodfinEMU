...
;			buffcrc = buffcrc2;
		...
		if (!creature->checkCooldownRecovery("avoid_incapacitation")) {
			StringIdChatParameter stringId;

			Time* cdTime = creature->getCooldownTime("avoid_incapacitation");

			// Returns -time. Multiple by -1 to return positive.
			int timeLeft = floor((float)cdTime->miliDifference() / 1000) *-1;

			creature->sendSystemMessage("Feign Death in 5 Minute Cooldown Window.");
		...
		creature->setRootedState(180);

		creature->addCooldown("avoid_incapacitation", 300 * 1000);
