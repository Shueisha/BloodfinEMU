...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		String syntaxerror = "You cant make that sick";
		...
		try {
			...
			targetPlayer->doAnimation("heavy_cough_vomit");

			if(fullReason.toString().isEmpty()) {
				targetPlayer->sendSystemMessage("You have lost control and are throwing up because of a force attack by " + player->getFirstName() + ".");
				player->sendSystemMessage(targetPlayer->getFirstName() + " is now puking because of you, arent you proud?");
