	...
	if (!(stateBitmask & state)) {
		...
		if (notifyClient) {
			...
			case CreatureState::FROZEN:
				...
				playEffect("clienteffect/cbt_friendlyfire_warn.cef");
				playEffect("clienteffect/droid_effect_dry_ice.cef");
				playEffect("clienteffect/stormtrp_movealng.cef");
				playEffect("clienteffect/thm_max_rebo_song_1.cef");
				break;
...
void CreatureObjectImplementation::setSnaredState(int durationSeconds) {
	if (!hasState(CreatureState::IMMOBILIZED)) {
		...
		state->setSpeedMultiplierMod(0.1);
