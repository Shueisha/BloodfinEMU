...
void PlayerObjectImplementation::notifySceneReady() {
	...
	ManagedReference<ChatRoom*> galaxyChat = chatManager->getGalaxyRoom();
	if( galaxyChat == NULL )
		return;

	galaxyChat->sendTo(creature);
	galaxyChat->addPlayer(creature);
...
void PlayerObjectImplementation::doForceRegen() {
	...
	if (getForcePower() < 0)
		setForcePower(0);
	int rankMod = 1;
	int rankModD = (creature->getSkillMod("force_power_dark") * .1);
	int rankModL = (creature->getSkillMod("force_control_light") * .1);
	...
	if (creature->isMeditating() && !creature->isInCombat())
		modifier = 7.f;

	if (creature->isKnockedDown() && creature->isInCombat())
		modifier = .05f;
