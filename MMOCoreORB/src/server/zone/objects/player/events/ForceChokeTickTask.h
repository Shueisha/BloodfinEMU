...
public:
	...
	void run() {
		//Rank Bonus
		int rankMod = 1;
		int rankModD = (creature->getSkillMod("force_power_dark") * 10);
		int rankModL = (creature->getSkillMod("force_power_light") * 10);
		...
		if(creature != NULL && creatureTarget != NULL) {
			int amountOfTicks = 2;
			if (creature->hasSkill("force_rank_light_rank_04") || creature->hasSkill("force_rank_dark_rank_04"))
				amountOfTicks += 1;
			if (creature->hasSkill("force_rank_light_rank_07") || creature->hasSkill("force_rank_dark_rank_07"))
				amountOfTicks += 1;
			if (creature->hasSkill("force_rank_light_rank_10") || creature->hasSkill("force_rank_dark_rank_10"))
				amountOfTicks += 1;
			if (creature->hasSkill("force_rank_light_master") || creature->hasSkill("force_rank_dark_master"))
				amountOfTicks += 1;
			if (counter < amountOfTicks && !creatureTarget->isIncapacitated() && !creatureTarget->isDead() && (creatureTarget->getPvpStatusBitmask() != CreatureFlag::NONE)) {
				...
				creatureTarget->inflictDamage(creature, CreatureAttribute::MIND, mindDamage * .5, true);
