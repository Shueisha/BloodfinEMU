...
public:
	...
	void run() {
		...
		try {
			...
			//Rank Bonus
			int rankMod = 1;
			int rankModD = (player->getSkillMod("force_power_dark") * 10);
			int rankModL = (player->getSkillMod("force_control_light") * 10);
			...
			if (playerObject->getForcePower() >= playerObject->getForcePowerMax())
				return;
			//if ((playerObject->getForcePower() - playerObject->getForcePower()) < forceBonus)
			...
			playerObject->setForcePower(playerObject->getForcePower() + rankMod);
