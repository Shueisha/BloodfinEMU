...
int PlayerContainerComponent::notifyObjectInserted(SceneObject* sceneObject, SceneObject* object) {
	...
	if (ghost && ghost->isJedi()) {
		if (object->isRobeObject() && cast<RobeObject*>( object)->getSkillRequired() != "") {
			VisibilityManager::instance()->increaseVisibility(creo);
			ghost->setForcePowerMax(ghost->getForcePowerMax() + 250, true);
			ghost->setForcePowerRegen(ghost->getForcePowerRegen() + 10);
		}

		if (object->isWeaponObject() && cast<WeaponObject*>(object)->isJediWeapon())
			VisibilityManager::instance()->increaseVisibility(creo);
