...
int PlayerManagerImplementation::MAX_CHAR_ONLINE_COUNT = 6;
...
void PlayerManagerImplementation::killPlayer(TangibleObject* attacker, CreatureObject* player, int typeofdeath) {
	...
	player->getPlayerObject()->setFoodFilling(0, true);
	player->getPlayerObject()->setDrinkFilling(0, false);
...
void PlayerManagerImplementation::sendActivateCloneRequest(CreatureObject* player, int typeofdeath) {
	...
	promptText << "@base_player:revive_closest : " << closestName << "\n"
			...
			<< "Bloodfin has claimed your soul. Select your option and click OK.";
	...
	if(ghost->getJediState() >= 2 || ghost->getAdminLevel() > 0)
		cloneMenu->addMenuItem("Bloodin Random Jedi Respawn Point", 1001); // Fake object ID to check for in sendPlayerToCloner
...
void PlayerManagerImplementation::sendPlayerToCloner(CreatureObject* player, uint64 clonerID, int typeofdeath) {
	...
	if(clonerID == 1001) // Fake obj id for jedi respawn
	{
		info("Attempting to send player to a random location via jedi respawn");

		Zone* zone = player->getZone();
		player->switchZone(zone->getZoneName(), System::random(5000), 0, System::random(5000), 0);
	...
	else
	{
		...
		if (preDesignatedFacility != NULL && preDesignatedFacility == cloningBuilding) {
			// bind removed
			player->sendSystemMessage("Your bind point will remain here until you change it");
...
/*
 * If (jedi state 4 or higher and (dark novice or light novice))
 * 		setOvert
 *
 * else
 * 		onleave
 */
	if (ghost->getJediState() >= 4 && (player->hasSkill("force_rank_dark_novice") || player->hasSkill("force_rank_light_novice"))) {
	...
	//Skyyyr Jedi experience loss.
	int jSkill = 0;
	//Lightsaber
	if (player->hasSkill("force_discipline_light_saber_novice"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_light_saber_master"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_light_saber_one_hand_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_light_saber_two_hand_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_light_saber_polearm_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_light_saber_technique_04"))
		jSkill += 1;
	//powers
	if (player->hasSkill("force_discipline_powers_novice"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_powers_master"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_powers_lightning_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_powers_mental_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_powers_debuff_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_powers_push_04"))
		jSkill += 1;
	//healing
	if (player->hasSkill("force_discipline_healing_novice"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_healing_master"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_healing_damage_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_healing_wound_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_healing_other_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_healing_states_04"))
		jSkill += 1;
	//enhancements
	if (player->hasSkill("force_discipline_enhancements_novice"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_enhancements_master"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_enhancements_movement_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_enhancements_protection_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_enhancements_resistance_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_enhancements_synergy_04"))
		jSkill += 1;
	//defender
	if (player->hasSkill("force_discipline_defender_novice"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_defender_master"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_melee_defense_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_ranged_defense_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_force_defense_04"))
		jSkill += 1;
	if (player->hasSkill("force_discipline_preternatural_defense_04"))
		jSkill += 1;
	int xpLoss = 10000;
	if(jSkill < 0)
		xpLoss = 0;
	...
	if (ghost->getJediState() > 1)
		awardExperience(player, "jedi_general", ((xpLoss * -1) * jSkill), true);
	//10 % roll base
	int rand =  System::random(9);
	if (ghost->getExperience("jedi_general") <= -1 && ghost->getJediState() > 1) {
		if (rand == 0) {
			player->removeSkill("force_discipline_healing_master");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
		if (rand == 1) {
			player->removeSkill("force_discipline_light_saber_master");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
		if (rand == 2) {
			player->removeSkill("force_discipline_defender_master");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
		if (rand == 3) {
			player->removeSkill("force_discipline_powers_master");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
		if (rand == 4) {
			player->removeSkill("force_discipline_enhancements_master");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
		if (rand == 5) {
			player->removeSkill("force_discipline_healing_damage_04");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
		if (rand == 6) {
			player->removeSkill("force_discipline_healing_states_04");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
		if (rand == 7) {
			player->removeSkill("force_discipline_defender_melee_defense_04");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
		if (rand == 8) {
			player->removeSkill("force_discipline_defender_ranged_defense_04");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
		if (rand == 9) {
			player->removeSkill("force_discipline_enhancements_protection_04");
			SkillModManager::instance()->verifySkillBoxSkillMods(player);
		}
	}
}
...
void PlayerManagerImplementation::enhanceCharacter(CreatureObject* player) {
	...
	if (message && player->isPlayerCreature())
		player->sendSystemMessage("Bloodfin has smiled upon you with free buffs..try not to die.");
}
