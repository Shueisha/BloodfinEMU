...
int CombatManager::getAttackerAccuracyModifier(CreatureObject* attacker, WeaponObject* weapon) {
	...
	//Rank Bonus
	int rankMod = 1;
	int rankModD = (attacker->getSkillMod("force_manipulation_dark") * .5);
	int rankModL = (attacker->getSkillMod("force_manipulation_light") * .5);
...
int CombatManager::getDefenderSecondaryDefenseModifier(CreatureObject* defender) {
	//Skyyyr saber_block intimidate fix
	if (!defender->getSkillMod("saber_block") && defender->isIntimidated())
		return 0;
...
float CombatManager::getDefenderToughnessModifier(CreatureObject* defender, int attackType, int damType, float damage) {
	...
	int lsToughness = defender->getSkillMod("lightsaber_toughness");
if (lsToughness == 0){
	...
	//Rank Bonus
	int rankMod = 1;
	int rankModD = (defender->getSkillMod("force_power_dark") * .15);
	int rankModL = (defender->getSkillMod("force_control_light") * .15);
...
int CombatManager::getArmorReduction(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defender, float damage, int poolToDamage, const CreatureAttackData& data){
	...
	if (data.getAttackType() == CombatManager::FORCEATTACK) {
		...
		if (forceFeedback > 0)
			attacker->inflictDamage(defender, System::random(2) * 3, rawDamage - (damage *= 1.f - (forceFeedback / 250.f)), true);
		...
		//Rank Bonus
		int rankMod = 1;
		int rankModD = (defender->getSkillMod("force_power_dark") * .5);
		int rankModL = (defender->getSkillMod("force_control1_light") * .5);
		...
		if (forceArmor > 0)
			defender->notifyObservers(ObserverEventType::FORCEBUFFHIT, attacker, rawDamage - (damage *= 1.f - (forceArmor / (100.f - rankMod))));
	}
...
void CombatManager::applyStates(CreatureObject* creature, CreatureObject* targetCreature, const CreatureAttackData& data) {
	...
	for (int i = 0; i < stateEffects->size(); i++) {
		...
		if (!failed) {
			...
			if (rankModL > rankModD)
				rankMod = rankModL;
			if (targetCreature->getSkillMod("jedi_state_defense") > 0)
				targetDefense += targetCreature->getSkillMod("jedi_state_defense");
				targetDefense += rankMod;
