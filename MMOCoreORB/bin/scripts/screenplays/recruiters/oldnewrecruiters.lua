...
function recruiter_convo_handler:runScreenHandlers(conversationTemplate, conversingPlayer, conversingNPC, selectedOption, conversationScreen)
	...
	elseif (screenID == "accepted_go_overt") then
		...
		createEvent(30, "recruiter_convo_handler", "handleGoOvert", conversingPlayer)
	elseif (screenID == "accepted_go_covert") then
		...
		createEvent(300, "recruiter_convo_handler", "handleGoOnLeave", conversingPlayer)
	...
	elseif (screenID == "accepted_resign") then
		if (playerObjectPointer ~= nil) then
			...
			if (playerObject:isOvert()) then
				playerObject:setFactionStatus(1)
				createEvent(300, "recruiter_convo_handler", "handleResign", conversingPlayer)
	...
	elseif (screenID == "accepted_resume_duties") then
		...
		createEvent(300, "recruiter_convo_handler", "handleGoOvert", conversingPlayer)
...
--	local playerObject = LuaPlayerObject(playerObjectPointer)
	...
	if (conversingPlayer:hasSkill("force_title_jedi_rank_03")) then
		return convoTemplate:getScreen("greet_hated")
	end

	if (conversingPlayer:hasSkill("force_rank_light_novice")) then
		return convoTemplate:getScreen("greet_hated")
	end

	if (conversingPlayer:hasSkill("force_rank_dark_novice")) then
		return convoTemplate:getScreen("greet_hated")
	end
...
function recruiter_convo_handler:awardItem(player, itemstring)
	...
	itemcost  = math.ceil(itemcost *  getGCWDiscount(player) * self:getSmugglerDiscount(creatureObject))
