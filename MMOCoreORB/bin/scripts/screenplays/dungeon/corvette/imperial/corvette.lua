...
spawnMapImperial =  {
		...
		{"rebel_commandant", 0, -2.6, 0, 60.5, 170, 4336203},
		{"rebel_ensign", 0, 5.0, 0, 18.1, 80, 4336201},
		{"rebel_ensign", 0, -0.1, 0, 9.4, -10, 4336199},
		{"rebel_ensign", 0, 0, 0, 23.3, 170, 4336199},

		{"rebel_ensign", 0, 10, 0, 17.6, 170, 4336208},

		{"rebel_commandant", 0, -2.9, 12, -16.3, 170, 4336216},
		{"rebel_commandant", 0, -2.9, 12, 14.9, 170, 4336216},
		{"rebel_admiral", 0, 1, 12, -8.2, -10, 4336217},
		{"rebel_commandant", 0, 3, 12, -0.7, 170, 4336217},

		{"rebel_admiral", 0, 2.8, 12, 4.1, -10, 4336218},
		...
		{"rebel_commandant", 0, -4.1, -12, 6.3, -100, 4336234},

		{"rebel_ensign", 0, -25, -12, 28, 80, 4336235},

		{"rebel_commandant", 0, -6.4, -6.7, -36, -100, 4336223},
		...
		{"rebel_commandant", 0, -21.1, -12, 68.3, 170, 4336242},
		{"rebel_sailor", 0, -24.6, -12, 64.1, 80, 4336242},
		{"rebel_sailor", 0, -17.4, -12, 64.1, -100, 4336242},

		{"rebel_commandant", 0, 0, -12, 100.7, 170, 4336248},

		{"rebel_commandant", 0, -17, -12, 112.9, 130, 4336249},
		{"rebel_commandant", 0, 17, -12, 112.9, -150, 4336249},

		{"rebel_commandant", 0, 0, -12, 117.4, 170, 4336250},
		{"rebel_commandant", 0, -5.8, -12, 127.5, 170, 4336250},

		{"rebel_commandant", 0, -5.4, -12, 133.6, 170, 4336251},
		{"rebel_commandant", 0, 5.6, -12, 132.7, -100, 4336251},

		{"rebel_commandant", 0, 0, -24, 98.3, -10, 4336254},

		{"rebel_commodore", 0, 3.3, -27, 75.6, 157, 4336256},
		{"rebel_commandant", 0, 3.2, -27, 78.6, 45, 4336256},

		{"rebel_commandant", 0, 0.7, 0, 111.5, -100, 4336257},
	...
	--"Rebel Commander", 0, -5.8, 0, 133.1, -10, 4336261},	
		...
		{"rebel_admiral", 0, 0, 0, 150.3, -10, 4336262},
...
globalSceneObjectsImperial =  {
		-- Format is: object (string), x, z, y, cell, ox, oz, oy, ow, custom name, custom object menu component.
		{"object/tangible/dungeon/wall_terminal_s3.iff",20,2,45,4336206,-1,0,0,1,"Escape Pod","escape_pod_menu_component"}
	},

-- Scene objects spawned in destroy-type missions only for the Imperial building.
destroySceneObjectsImperial = {
		{"object/tangible/dungeon/wall_terminal_s1.iff",-10,-0,-10.2,4336206,0,0,0,0,"Engine Controls","corvette_destroy_object_menucomponent"}
},
spawnMapNeutral =  {
		{"rebel_ensign", 0, -10, 0, -10.2, -10, 4336209},
		{"rebel_ensign", 0, 4.9, 0, -30.9, -10, 4336213},
		...
		{"rebel_commandant", 0, 14.3, 0, 51.6, -100, 4336203},
		{"rebel_commandant", 0, -2.6, 0, 60.5, 170, 4336203},
		{"corvette_crew", 0, 5.9, 0, 18.1, 80, 4336201},
		{"rebel_ensign", 0, -0.1, 0, 9.4, -10, 4336199},
		{"rebel_ensign", 0, 0, 0, 23.3, 170, 4336199},
...
spawnMapRebel =  {
		...
		{"rebel_commandant", 0, 14.3, 0, 51.6, -100, 4336203},
		{"rebel_commandant", 0, -2.6, 0, 60.5, 170, 4336203},
		{"imperial_trooper", 0, -10.9, 0, 18.1, 80, 4336201},
		{"rebel_ensign", 0, -0.1, 0, 9.4, -10, 4336199},
		{"rebel_ensign", 0, 0, 0, 23.3, 170, 4336199},
		...
		for i = 0, gSize do
			...
			if (groupMember ~= nil) then
				if (groupMember ~= pPlayerOwner) then
					...
					createEvent(36000, "CorvetteScreenPlay", "instanceFailed", groupMember)
				else
					ownerCreature:sendSystemMessage("You have one hour to complete your objective.")
...
function CorvetteScreenPlay:instanceFailed(pPlayer)
	if (pPlayer == nil) then
		createEvent(300, "CorvetteScreenPlay", "instanceFailed", pPlayer)
	end
...
function CorvetteScreenPlay:EngineSuiCallback(pCreature, pSui, cancelPressed, otherPressed, okPressed)
	...
	-- Send the message.
		local setting = 40
		player:sendSystemMessage("@dungeon/corvette:engine_set" .. setting)
...
function CorvetteScreenPlay:setShipComponent(componentName, setting, pCreature)
	...
	if (componentName == "engine") then
		engineSetTo = setting
		player:sendSystemMessage("@dungeon/corvette:engine_is" .. setting)
	end
...
function corvette_destroy_object_menucomponent:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	...
	local suiManager = LuaSuiManager()
		...
		-- Engine.
		if (luaSceneObject:getTemplateObjectPath() == "object/tangible/dungeon/corellian_corvette/corvette_search_imperial_destroy_03.iff") then
			local suiManager = LuaSuiManager()
