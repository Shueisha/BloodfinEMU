...
function StrongholdScreenPlay:spawnSceneObjects()
	...
	local pCollector4 = spawnSceneObject("corellia", "object/tangible/furniture/imperial/data_terminal_s1.iff", 4629.1, 25, -5784, 0, 0, 0, 0, 0)
	local collector = LuaSceneObject(pCollector4)
	local col2creo = LuaCreatureObject(pCollector4)
	col2creo:setOptionsBitmask(128)
	collector:setCustomObjectName("\\#ee3377Travel to Dathomir Imperial Prison")
	createObserver(OBJECTRADIALUSED, "StrongholdScreenPlay", "teleportDathprison", pCollector4)	
...
function StrongholdScreenPlay:teleportDathprison(pCollector, pPlayer)
	local playerfaction = LuaCreatureObject(pPlayer)
	if (playerfaction:isImperial() == true) then
		local player = LuaSceneObject(pPlayer)
		player:switchZone("dathomir", -6228.6, 120, 942.1, -74)
	else
		local playerm = LuaCreatureObject(pPlayer)
		playerm:sendSystemMessage("You are not authorized to use this terminal")
	end
